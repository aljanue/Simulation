public class DeformableObject
{
   ...
   DeformableObject(SpringLayout layout, float x, color c)
   {
    _numNodesH = N_H;
    _numNodesV = N_V;
    _sepH = D_H;
  ...
  void createForm(float x){
      for (int j = 0; j < _numNodesV; j++)
      {  
        for (int i = 0; i < _numNodesH; i++)
        {
          PVector pos = new PVector(i*_sepH - _numNodesH*_sepH/2+x, 0, j*_sepV+height/4);
          _nodes[i][j] = new Particle(pos, new PVector(0, 0, 0), MASS_NODES, _noGravity, false);
        }
      }
      for (int y = 0; y < _numNodesV; y++)
        _nodes[0][y].setClamped(true);
  }
